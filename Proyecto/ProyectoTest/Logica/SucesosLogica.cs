...
{
    public class SucesosLogica
    {
        private static SucesosLogica _instancia = null;

        public SucesosLogica()
        {

        }

        public static SucesosLogica Instancia
        {
            get
            {
                if (_instancia == null)
                {
                    _instancia = new SucesosLogica();
                }
        ...
        public List<Sucesos> Listar()
        {

            List<Sucesos> rptListaProducto = new List<Sucesos>();
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                ...
                try
                {
                    ...
                    while (dr.Read())
                    {
                        rptListaProducto.Add(new Sucesos()
                        {
                            IdProducto = Convert.ToInt32(dr["IdProducto"].ToString()),
                            Nombre = dr["Nombre"].ToString(),
                            Descripcion = dr["Descripcion"].ToString(),
                            oMarca = new Medios() { IdMarca = Convert.ToInt32(dr["IdMarca"].ToString()),Descripcion = dr["DescripcionMarca"].ToString() },
        ...
        public int Registrar(Sucesos oProducto)
        {
            int respuesta = 0;
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
        ...
        public bool Modificar(Sucesos oProducto)
        {
        ...
        public bool ActualizarRutaImagen(Sucesos oProducto)
        {
            bool respuesta = true;
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
